...
namespace coen
{
    namespace pinc
    {
        ...
        // gather() actually returns a response from all awaited coroutines and
        // suspends the caller, the wait() just suspends the caller but doesn't
        // return anything. Kind of duplicate no? What if you just ignore the
        // response of gather() ?
        template <typename t_TYPE = void>
        Awaitable<> wait();
